...
export default class GreedyTakerCautiousDiscard implements Player {
    private priorities: Priority[];

    constructor(priorities: Priority[] = ['7velo', 'escoba', 'cartas', 'oros']) {
    ...
    makePlay(gameState: {
        ...
        for (let handIndex = 0; handIndex < hand.length; handIndex++) {
            ...
            combinations.push(...sumTo(15 - card.value, board).map(
                comb => [card, ...comb]
            ));
        }
        ...
        if (combinations.length > 0) {
            ...
            this.priorities.forEach( priority => {
                if (combinations.length == 1) return combinations[0].map( card => card.index);
            ...
            if (not7Velo.length == 0) return [0]; // discard 7 velo

            const lowSum = not7Velo.filter( card => card.value + board.reduce( (prev, curr) => prev + curr.value, 0) < 5);
            if (lowSum.length > 0) return [hand[0].index] // discard any low number
    ...
    aplyPriority(priority: Priority, combinations: Card[][], boardLength: number): Card[][] {
        if (priority == 'cartas') {
            ...
            const comb7Velo = combinations.filter( comb => 
                comb.some( card => card.value == 7 && card.suit == 'oro')
            );
            if (comb7Velo.length != 0) return comb7Velo;
            else return combinations
        } else if (priority == 'oros') {
            const max = Math.max(...combinations.map( comb => comb.reduce( (acc, curr) => {
                if (curr.suit == 'oro') return acc + 1;
                else return acc;
            }, 0)));
            return combinations.filter( comb => {
                const oros: number = comb.reduce( (acc, curr) => {
                    if (curr.suit == 'oro') return acc + 1;
                    else return acc;
                }, 0)
                return oros == max;
